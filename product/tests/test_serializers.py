...
class ProductSerializerTest(APITestCase):
    def test_product_serialization(self):
        ...
        product = Product.objects.create(
            title="Test Book", description="Test Description", price=100, active=True
        )
        ...
        )  # Testa o campo 'description' de Product
        ...
        self.assertEqual(
            len(data["category"]), 1
        )  # Testa a relação 'category' (ManyToMany) do Product
        self.assertEqual(
            data["category"][0]["title"], "Fiction"
        )  # Testa o campo 'title' da Category associada
